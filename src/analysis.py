...
parser.add_argument('-r', '--regression', dest='regression_flag', action='store_true')
...
if args.describe_flag:
    print(data_day[['temp','hum','casual','registered','cnt']].describe())
    print('\nNull values:')
    nulls = data_day.isnull().sum().sort_values(ascending=False)/data_day.shape[0]
    print(nulls)
    print(data_hour[['temp','hum','casual','registered','cnt']].describe())
    print('\nNull values:')
...
### end plot if
if args.regression_flag:
    print('Regression models:')
    print('Daily results')
    y_test, predictions = mod.train_test_models(data_day)

    mod.plot_linearity_assumption(y_test, predictions['lm'], 'daily Linear Model', output_dir)
    mod.plot_linearity_assumption(y_test, predictions['l'], 'daily Lasso', output_dir)
    mod.plot_linearity_assumption(y_test, predictions['r'], 'daily Ridge', output_dir)
    mod.plot_linearity_assumption(y_test, predictions['en'], 'daily Elastic Net', output_dir)
    mod.plot_linearity_assumption(y_test, predictions['rf'], 'daily Random Forest', output_dir)
    ...
    mod.plot_linearity_assumption(y_test, predictions['lm'], 'daily Linear Model', output_dir)
    mod.plot_linearity_assumption(y_test, predictions['rf'], 'daily Random Forest', output_dir)
    mod.plot_dist_predition(y_test, predictions['lm'], 'daily Linear Model', output_dir)
    mod.plot_dist_predition(y_test, predictions['rf'], 'daily Random Forest', output_dir)

    print('Hourly results')
    y_test, predictions = mod.train_test_models(data_hour)

    mod.plot_linearity_assumption(y_test, predictions['lm'], 'hourly Linear Model', output_dir)
    mod.plot_linearity_assumption(y_test, predictions['l'], 'hourly Lasso', output_dir)
    mod.plot_linearity_assumption(y_test, predictions['r'], 'hourly Ridge', output_dir)
    mod.plot_linearity_assumption(y_test, predictions['en'], 'hourly Elastic Net', output_dir)
    mod.plot_linearity_assumption(y_test, predictions['rf'], 'hourly Random Forest', output_dir)
    ...
    mod.plot_linearity_assumption(y_test, predictions['lm'], 'hourly Linear Model (opt)', output_dir)
    mod.plot_linearity_assumption(y_test, predictions['rf'], 'hourly Random Forest (opt)', output_dir)
    mod.plot_dist_predition(y_test, predictions['lm'], 'hourly Linear Model (opt)', output_dir)
    mod.plot_dist_predition(y_test, predictions['rf'], 'hourly Random Forest (opt)', output_dir)
